#!/bin/bash
#USAGE ./bowtie_paired.sh reference reads1 reads2 result threads hashsz 
threads=4
hashsz=10 
if [ $# -ge 5 ]; then
    threads=$5
...
date | tee -a $out/log && \
bowtie -t -q -p $threads -a --fr --minins 0 --maxins 500 --sam $out/$ref_base.ind -1 $2 -2 $3 > $out/$ref_base.sam && \
